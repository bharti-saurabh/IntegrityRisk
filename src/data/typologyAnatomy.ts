...
export const ANATOMY: AnatomyModule[] = [
  ...
  {
    typ: "CARD_SURCHARGE",
    alias: "Card-Surcharge Abuse / Fee Non-compliance",
    oneLiner: "A checkout fee is added to card payments in a way the brand rules don't permit — over the cap, on debit/prepaid, or undisclosed.",
    definition:
      "Surcharging a card transaction is allowed only within strict limits: it may not exceed the merchant's cost of acceptance (the brand cap, typically ~3–4%), it is prohibited on debit and prepaid cards, and it must be disclosed at the point of entry and on the receipt. A merchant that breaks any of these adds an unexpected fee the cardholder didn't agree to — which surfaces as 'not recognized' / unauthorized-fee disputes, chargebacks and fee reversals.",
    variants: [
      { name: "Over-cap surcharge", tell: "The surcharge exceeds the brand's cost-of-acceptance cap — the cardholder pays more than the rules allow." },
      { name: "Prohibited debit / prepaid", tell: "A surcharge is applied to debit or prepaid cards, where surcharging is banned outright." },
      { name: "Undisclosed surcharge", tell: "No point-of-sale or receipt disclosure, so the fee is a surprise and gets disputed as unauthorized." },
      { name: "Blended / hidden fee", tell: "The surcharge is folded into the ticket rather than shown as a separate line, obscuring it from the cardholder and the network." },
    ],
    identificationSignals: [
      { label: "'Not recognized' unexpected-fee dispute rate", feature: "notRecognizedDisputeRate" },
      { label: "Overall dispute / chargeback rate", feature: "disputeRate" },
      { label: "Fee-reversal (refund) rate", feature: "refundRate" },
    ],
    validationLadder: [
      {
        tier: "signal",
        method: "Fee-dispute scoring",
        detail:
          "Score the 'merchant not recognized' / unexpected-fee dispute rate alongside the overall chargeback and fee-reversal rates — the behavioral fallout of a non-compliant surcharge.",
        inPlatform: true,
      },
      {
        tier: "corroboration",
        method: "Surcharge re-derivation",
        detail:
          "Sample card transactions and re-derive the effective surcharge as a share of the base ticket; compare it against the cost-of-acceptance cap and check whether debit/prepaid BINs were surcharged.",
        inPlatform: true,
      },
      {
        tier: "corroboration",
        method: "Disclosure & receipt review (OSINT / field)",
        detail:
          "Verify whether the surcharge is disclosed at the point of entry and itemized on the receipt as required by the brand rules and applicable state law.",
        inPlatform: false,
      },
      HUMAN,
    ],
    implications: [
      { area: "Consumer", text: "Cardholders are charged a fee they didn't expect or agree to → 'not recognized' disputes and direct financial harm." },
      { area: "Regulatory", text: "Brand surcharge rules and state surcharge statutes; unfair-practices exposure where the fee is undisclosed." },
      { area: "Operational", text: "Elevated chargebacks and fee reversals raise the merchant's dispute ratio toward monitoring-program thresholds." },
    ],
    regulatoryHooks: ["Visa/MC surcharge rules", "Cost-of-acceptance cap", "State surcharge statutes"],
  },
  {
